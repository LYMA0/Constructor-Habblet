function criandoElementoConstructor(novaJanela, divAlvo){
     novaJanela.innerHTML = htmlConstructor;
     divAlvo.appendChild(novaJanela); // Criando a nova janela
     statusConstructor = false; // Muda a variavel de controle para poder abrir somente uma janela
     ...
     esperarElemento("#enviar-up").then(element => {
          const input = document.querySelector("#up");
          const string = "up";
          element.addEventListener("click", function(){
          executarComando(string, input.value);
          });
     });

     // Criando ação no botão SECUNDARIO UP
     esperarElemento("#opcao-up").then(element => {
          const string = "up";
          element.addEventListener("click", function(){
     ...
     esperarElemento("#enviar-state").then(element => {
     ...
     // Criando ação no botão SECUNDARIO STATE
     esperarElemento("#opcao-state").then(element => {
          const string = "state";
          element.addEventListener("click", function() {
          executarComando(string, " ");
          });
     ...
     esperarElemento("#enviar-spin").then(element => {
          const input = document.querySelector("#spin");
          const string = "spin";
          element.addEventListener("click", function() {
          executarComando(string, input.value);
          });
     ...
     // Criando ação no botão SECUNDARIO SPIN
     esperarElemento("#opcao-spin").then(element => {
          const string = "spin";
          element.addEventListener("click", function() {
          executarComando(string, " ");
          });
     ...
     // Criando strings para as funções a seguir
     const stringTile = "tile";
     const stringCopy = "copy";
     const stringPlace = "place";
     const stringPick = "pick";
     const stringMove = "move";

     // Criando ação no botão TILE COPY
     esperarElemento('#enviar-copy').then(element => {
          element.addEventListener("click", function() {
          executarComando(stringTile, stringCopy);
          });
     })

     // Criando ação no botão TILE PLACE
     esperarElemento('#enviar-place').then(element => {
          element.addEventListener("click", function() {
          executarComando(stringTile, stringPlace);
          });
     })

     // Criando ação no botão TILE PICK
     esperarElemento('#enviar-pick').then(element => {
          element.addEventListener("click", function() {
          executarComando(stringTile, stringPick);
          });
     })

     // Criando ação no botão TILE MOVE
     esperarElemento('#enviar-move').then(element => {
          element.addEventListener("click", function() {
          executarComando(stringTile, stringMove);
          });
     })
}
